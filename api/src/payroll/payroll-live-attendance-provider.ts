...
export function verifyPayrollAttendanceEvidence(employeeId: number, periodStart: string, periodEnd: string,
  schedule: PayrollLiveSourceSection, employment: PayrollLiveSourceSection, evidence: PayrollAttendanceEvidenceRows, now = new Date()): PayrollLiveSourceSection {
  ...
  for (const scheduled of scheduleDays) {
    ...
    const raw = punches.filter(p => localDate(p.punchTime) === date).sort((a, b) => a.punchTime.getTime() - b.punchTime.getTime() || a.id - b.id)
    ...
    if (!instant(stored.computedAt) || stored.computedAt > now || date >= localDate(now) ||
      (stored.status === 'absent' && localDate(stored.computedAt) <= date)) fail('UNSUPPORTED', 'ATTENDANCE_DAY_NOT_CLOSED', 'اليوم لم ينتهِ أو لا يوجد إثبات غياب محفوظ بعد انتهائه')
    ...
    if (!timing || !clockValid(timing.startTime) || !clockValid(timing.endTime) || timing.startTime >= timing.endTime) fail('UNSUPPORTED', 'ATTENDANCE_OVERNIGHT_PROOF_UNSUPPORTED', 'إثبات اختيار بصمات الوردية الليلية يحتاج لقطة مدخلات محفوظة')
    ...
    const precise = (v: Date) => v.getHours() * 60 + v.getMinutes() + v.getSeconds() / 60 + v.getMilliseconds() / 60000
    const start = minute(timing.startTime), finish = minute(timing.endTime)
    ...
    const early = !timing.flexEnabled && last ? Math.max(0, finish - minute(clock(last))) : 0
    ...
    const rawMs = first && lateApplies ? Math.max(0, ((first.getHours() * 60 + first.getMinutes()) - start) * 60000 + first.getSeconds() * 1000 + first.getMilliseconds()) : 0
...
export async function readPayrollLiveAttendance(em: EntityManager, employeeId: number, periodStart: string, periodEnd: string,
  ...
  const params = [employeeId, periodStart, periodEnd, bound(periodStart, -2), bound(periodEnd, 3)]
  try {
    ...
    const leaves = await em.query(`SELECT TOP (5001) [id],[employeeId],[requestId],CONVERT(varchar(10),[fromDate],23) AS [fromDate],
      ...
      WHERE [employeeId]=@0 AND [status]='APPROVED' AND [fromDate]<=@2 AND [toDate]>=@1 ORDER BY [id]`, params)
    const corrections = await em.query(`SELECT TOP (5001) c.[id],c.[employeeId],c.[requestId],CONVERT(varchar(10),c.[date],23) AS [date],c.[correctedPunch],
      ...
      WHERE c.[employeeId]=@0 AND c.[date]>=@1 AND c.[date]<=@2 ORDER BY c.[id]`, params)
    ...
    return verifyPayrollAttendanceEvidence(employeeId, periodStart, periodEnd, schedule, employment, { days, punches, leaves, corrections, requests, exemptions })
  } catch (error: any) {
