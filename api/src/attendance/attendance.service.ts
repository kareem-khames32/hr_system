...
export class AttendanceService {
  ...
  async daily(user: JwtPayload, date: string) {
    ...
    const scoped = await this.employees.find({ where: scope === null ? {} : { branchId: branchIdIn(scope) } })
  ...
  async liveAbsences(date: string, scope: BranchScope): Promise<Array<AttendanceDay & { live: true }>> {
    ...
    const active = await this.employees.find({ where: { isActive: true, ...(scope === null ? {} : { branchId: branchIdIn(scope) }) } })
