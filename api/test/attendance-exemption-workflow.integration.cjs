...
test('S24 dates: a pending request created in the previous cycle stays approvable after a new cycle starts; the previous cycle (paid next month) is accepted, anything older stays refused', async () => {
  const emp = await employee()
  ...
  const older = new Date(`${previousStart}T12:00:00`)
  older.setMonth(older.getMonth() - 1)
  const olderStart = formatDate(older)
  ...
  // تبسيط الرواتب: مسير الفترة السابقة يُصرف بعد انتهائها، فطلب جديد يبدأ من أول الفترة السابقة مقبول ما دام مسيرها لم يُعتمد
  const paidNextMonth = await employee()
  const previousCycle = await create(paidNextMonth, { effectiveFrom: previousStart })
  assert.deepEqual([previousCycle.status, previousCycle.effectiveFrom], ['PENDING', previousStart])
  assert.equal((await approve(previousCycle)).status, 'APPROVED')
  const other = await employee()
  const refused = await request(hr, 'POST', '/attendance-exemptions', input(other, { effectiveFrom: olderStart }))
  assert.equal(refused.status, 400, JSON.stringify(refused.body))
  const retro = await legacyPending(other, { effectiveFrom: olderStart })
