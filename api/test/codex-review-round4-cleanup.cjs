...
;(async()=>{let pool;try{
 const files=fs.readdirSync(__dirname).filter(n=>/^codex-review-round4-results-.*\.cjs$/.test(n)),names=new Set()
 ...
 names.add('hr_decision_race_cbdce9ac8f56d8d5');
 ...
 const candidates=fs.readdirSync(__dirname).filter(n=>n.startsWith('codex-review-round4-')||n==='codex-review-round2-performance-round3-500-original.cjs').map(n=>path.join(__dirname,n))
 const report=path.resolve(__dirname,'../../CODEX_REVIEW_REPORT_4.md');if(fs.existsSync(report))candidates.push(report)
 ...
 fs.writeFileSync(path.join(__dirname,'codex-review-round4-cleanup-results.cjs'),'module.exports = '+JSON.stringify(result,null,2)+'\n')
 ...
 assert.equal(remaining.length,0);assert.equal(matches.length,0)
