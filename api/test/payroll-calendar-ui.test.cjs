...
test('scope guard does not treat star permissions as unlimited branch scope',()=>{
 const scoped={role:'hr',branchId:5,permissions:['*']};assert.equal(ui.calendarScopeWritable('GLOBAL',0,scoped),false);assert.equal(ui.calendarScopeWritable('BRANCH',5,scoped),true);assert.equal(ui.calendarScopeWritable('BRANCH',6,scoped),false);assert.equal(ui.calendarScopeWritable('GLOBAL',0,{...scoped,role:'super_admin'}),true);assert.equal(ui.calendarScopeWritable('GLOBAL',0,{...scoped,branchId:null}),false);assert.equal(ui.calendarScopeWritable('GLOBAL',0,{...scoped,branchId:null,scopeAllBranches:true}),true);const multi={...scoped,branchIds:[5,6]};assert.equal(ui.calendarScopeWritable('BRANCH',6,multi),true);assert.equal(ui.calendarScopeWritable('BRANCH',7,multi),false);assert.equal(ui.calendarScopeWritable('GLOBAL',0,multi),false);assert.equal(ui.calendarScopeWritable('BRANCH',5,{...scoped,branchIds:[]}),false);assert.equal(ui.calendarScopeWritable('GLOBAL',0,null),false)
})
