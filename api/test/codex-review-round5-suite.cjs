...
const reviewedTemporaryMigrations=new Set(['roles-scope-presets.integration.cjs','security-role-grants.integration.cjs','codex-review-round5-migrations.integration.cjs','user-branch-scopes.integration.cjs','holiday-work.integration.cjs','leave-year-end.integration.cjs','assets-branch.integration.cjs','leave-contract.integration.cjs','codex-review-round2-migration067.integration.cjs'])
...
stream.on('test:stdout', d => { const s=redact(d.message); stdout.push(s); if (/REVIEW_MEASURE|REVIEW_PROGRESS|cleanupVerified|BATCH_SHADOW|CR5_/.test(s)) console.log(s.trim()) })
...
stream.on('test:summary', d => {
  if (!d.file) {
    ...
    fs.writeFileSync(path.join(__dirname, `codex-review-round5-results-${suffix}.cjs`), 'module.exports = ' + redact(JSON.stringify(final, null, 2)) + '\n')
