...
test('نقاط الربط: تجسيد الغياب يتخطى أيام الإيقاف، والمسير يضيفها لعمود بدون راتب بسطر مستقل، والشاشات موصولة', () => {
  const attendance = fs.readFileSync(path.join(__dirname, '../src/attendance/attendance.service.ts'), 'utf8').replace(/\r\n/g, '\n')
  ...
  const payroll = fs.readFileSync(path.join(__dirname, '../src/payroll/payroll.service.ts'), 'utf8').replace(/\r\n/g, '\n')
  ...
  const list = fs.readFileSync(path.join(root, 'src/app/employees/page.tsx'), 'utf8').replace(/\r\n/g, '\n')
  assert.match(list, /label: 'إيقاف مؤقت'/); assert.match(list, /<EmployeeSuspensionDialog/)
  const profile = fs.readFileSync(path.join(root, 'src/app/employees/[id]/page.tsx'), 'utf8').replace(/\r\n/g, '\n')
  assert.match(profile, /سجل الإيقاف عن العمل/); assert.match(profile, /<EmployeeSuspensionDialog/)
  const migration = fs.readFileSync(path.join(root, 'docs/migrations/payroll/20260916_044_employee_suspension.sql'), 'utf8').replace(/\r\n/g, '\n')
...
test('شاشة الملف: الإيقاف المنتهي في السجل له زرار «إلغاء أو تعديل النهاية» يفتح نافذة التصحيح', () => {
  ...
  const dialog = fs.readFileSync(path.join(root, 'src/components/EmployeeSuspensionDialog.tsx'), 'utf8').replace(/\r\n/g, '\n')
