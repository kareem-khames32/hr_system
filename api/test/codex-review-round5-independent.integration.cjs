...
before(async()=>{
  f=await require('./codex-review-round5-fixture.cjs')('r5independent')
  ;[a,b,c]=await f.repo('Branch').save(['A','B','C'].map(code=>({code:'R5'+code,name:'Review '+code})))
  employee=await f.repo('Employee').save({employeeCode:'R5C',fullName:'FOREIGN REVIEW PERSON',branchId:c.id,joinDate:'2020-01-01',isActive:true,status:'active'})
  actor=await f.repo('User').save({email:'r5scope@review.invalid',displayName:'Scope A B',passwordHash:'fixture-only',role:'employee',branchId:a.id,scopeBranchIds:JSON.stringify([a.id,b.id]),permissions:JSON.stringify(['requests.view_all','requests.create_on_behalf','attendance_exemption.manage','attendance_exemption.view','attendance.view_all','employees.view'])})
  ;[chain1,chain2]=await f.repo('ApprovalChain').save([1,2].map(n=>({code:'R5CHAIN'+n,nameAr:'Review chain '+n,isActive:true,autoApprove:false})))
  await f.repo('ApprovalStep').save([chain1,chain2].map(ch=>({chainId:ch.id,stepOrder:1,approverRole:'hr'})))
  type=await f.repo('RequestType').save({code:'R5_TRAIN',nameAr:'Review training',category:'training',destinationHandler:'none',approvalChainId:chain1.id,isActive:true})
},{timeout:120000})
after(async()=>{if(f)await f.close()})

test('CR5 category config PATCH cannot evade its guard with SQL case folding',async t=>{
  await f.ok('PUT','/settings/request-categories/training/chain',{chainId:chain1.id,repointTypeIds:[type.id]})
  const configOnly=await f.repo('User').save({email:'r5config@review.invalid',displayName:'Configuration only',passwordHash:'fixture-only',role:'employee',scopeAllBranches:true,permissions:JSON.stringify(['settings.manage'])})
...
test('CR5 outside request and nonexistent request must not reveal existence',async t=>{
...
test('CR5 outside exemption target and missing target must not reveal existence',async t=>{
...
test('CR5 payroll separation licence cannot evade its specific permission by SQL case folding',async t=>{
  const {PAYROLL_SELF_APPROVAL_KEY}=require('../src/payroll/payroll-run-approval')
  await f.setting(PAYROLL_SELF_APPROVAL_KEY,'false')
  const configOnly=await f.repo('User').findOneByOrFail({email:'r5config@review.invalid'})
  ...
  await f.setting(PAYROLL_SELF_APPROVAL_KEY,'false')
  assert.equal(upper.status,403);assert.equal(stored.value,'false')
})

test('CR5 archived employee without an offboarding case is not absent in either attendance view',async t=>{
  const e=await f.repo('Employee').save({employeeCode:'R5_ARCH',fullName:'Review archived',branchId:a.id,joinDate:'2020-01-01',archivedAt:new Date('2026-09-10T12:00:00'),isActive:false,status:'archived'})
...
test('CR5 HR permission grants one financial effect for another employee, never own request or outside scope',async t=>{
  const people=await f.repo('Employee').save([a,b].map((branch,n)=>({employeeCode:'R5HR'+n,fullName:'Review HR '+n,branchId:branch.id,joinDate:'2020-01-01',isActive:true,status:'active'})))
  const hr=await f.repo('User').save({email:'r5hr@review.invalid',displayName:'HR permission holder',passwordHash:'fixture-only',role:'employee',branchId:a.id,scopeBranchIds:JSON.stringify([a.id,b.id]),employeeId:people[0].id,permissions:JSON.stringify(['approve.hr','requests.create_on_behalf','requests.view_all'])})
  const desk=await f.repo('User').save({email:'r5desk@review.invalid',displayName:'Request clerk',passwordHash:'fixture-only',role:'employee',branchId:a.id,scopeBranchIds:JSON.stringify([a.id,b.id]),permissions:JSON.stringify(['requests.create_on_behalf','requests.view_all'])})
  const financial=await f.repo('RequestType').save({code:'R5_CREDIT',nameAr:'Review financial credit',category:'financial',destinationHandler:'payroll_allowance',approvalChainId:chain1.id,isActive:true,requiredFields:JSON.stringify(['amount','description']),visibleTo:JSON.stringify({mode:'all',ids:[]})})
...
test('CR5 exemption subject cannot approve or reject an exemption created by someone else',async t=>{
  const person=await f.repo('Employee').save({employeeCode:'R5SOD',fullName:'Review exemption subject',branchId:a.id,joinDate:'2020-01-01',isActive:true,status:'active'})
  const subject=await f.repo('User').save({email:'r5sod@review.invalid',displayName:'Exemption subject',passwordHash:'fixture-only',role:'hr_manager',branchId:a.id,employeeId:person.id,permissions:JSON.stringify(['attendance_exemption.approve','attendance_exemption.approve_executive','attendance_exemption.view'])})
