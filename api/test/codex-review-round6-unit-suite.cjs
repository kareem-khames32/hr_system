...
stream.on('test:summary', d => {
  if (!d.file) { fs.writeFileSync(path.join(__dirname,'codex-review-round6-results-unit.cjs'),'module.exports = '+JSON.stringify({summary:d,failures,files:files.map(f=>path.basename(f)),excluded:[...excluded]},null,2)+'\n'); console.log(JSON.stringify({ summary: d, failures })); process.exitCode = d.success ? 0 : 1 }
})
