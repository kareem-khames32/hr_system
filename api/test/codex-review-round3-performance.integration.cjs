...
let f, active = null
const measurements=[]
const resultFile=path.join(__dirname,`codex-review-round3-performance-${process.env.REVIEW_PERF_SUFFIX||'results'}.cjs`)
...
before(async()=>{f=await require('./codex-review-round2-fixture.cjs')('r3performance')},{timeout:180000})
after(async()=>{
  fs.writeFileSync(resultFile,'module.exports = '+JSON.stringify(measurements,null,2)+'\n')
  if(f)await f.close()
})
test('CR3 PERF real attendance data copied only INTO disposable database; no source writes',async t=>{
  ...
  if(process.env.REVIEW_REAL_PAYROLL20==='true'){
    await require('./codex-review-round3-real20.cjs')({f,measure,t});return
  }
...
test('CR3 PERF calculate exactly 500 synthetic employees and independently reconcile 3900000',async t=>{
  ...
  assert.equal(accrualCount.computedDays,past?15000:1000)
