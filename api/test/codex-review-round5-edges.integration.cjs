...
const change={effectiveFrom:'2026-01-01',changeReason:'Independent fifth review dated rule'}
before(async()=>{
 f=await require('./codex-review-round5-fixture.cjs')('r5edges')
 ;[a,b,c]=await f.repo('Branch').save(['A','B','C'].map(code=>({name:'Review '+code,code:'R5E'+code,country:'EG'})))
 scoped=await f.repo('User').save({email:'scope@r5edges.invalid',displayName:'Review scoped',passwordHash:'fixture-only',role:'employee',branchId:a.id,scopeBranchIds:JSON.stringify([a.id,b.id]),permissions:JSON.stringify(['settings.manage','requests.view_all','employees.view','request_types.manage','approval_chains.manage'])})
 await f.setting('payroll.cycle_start_day',23);await f.setting('attendance.weekend_days','SAT')
},{timeout:120000})
after(async()=>{if(f)await f.close()})
async function employee(extra={}){return f.repo('Employee').save({employeeCode:'R5EDGE'+(++seq),fullName:'Review employee '+seq,branchId:a.id,joinDate:'2020-01-01',isActive:true,status:'active',...extra})}
...
test('CR5 financial and calendar Saturday occurrences are checked against enumerated dates, then versioned',async t=>{
...
test('CR5 service window ignores cancelled and pre-rehire offboarding, honors actual start and archive fallback',async t=>{
...
test('CR5 foreign work schedule and shift history must respect branch scope',async t=>{
...
test('CR5 request card masks confidential records and must not reveal the new foreign organization after transfer',async t=>{
 const e=await employee({jobTitle:'Original job'})
 const type=await f.repo('RequestType').save({code:'R5E_CARD',nameAr:'Review employee card',category:'training',destinationHandler:'none',isActive:true})
 ...
 assert.equal(historic.requester.orgHidden,true)
 for(const key of ['jobTitle','departmentName','branchName','teamName','directManagerName'])assert.equal(historic.requester[key],null,key)
 assert.equal(historic.requester.fullName,e.fullName);assert.equal(historic.requester.employeeCode,e.employeeCode)
})

test('CR5 unauthorized upper-case licence is blocked and real self-calculated payroll stays unapproved',async t=>{
 const {PAYROLL_SELF_APPROVAL_KEY}=require('../src/payroll/payroll-run-approval')
 const branch=await f.repo('Branch').save({code:'R5SODPAY',name:'Review self approval'})
 ...
 const user=await f.repo('User').save({email:'payroll@r5edges.invalid',displayName:'Review calculator with settings',role:'employee',passwordHash:'fixture-only',scopeAllBranches:true,permissions:JSON.stringify(['settings.manage','payroll.view','payroll.calculate','payroll.approve','payroll.pay'])})
 ...
 assert.equal(bypass.body.code,'PAYRUN-SOD-LICENCE-PERMISSION')
 assert.equal((await f.repo('RequestsConfig').findOneByOrFail({key:PAYROLL_SELF_APPROVAL_KEY})).value,'false')
 ...
 assert.equal(after.body.code,'PAYRUN-STATE-003');assert.equal(saved.status,'CALCULATED');assert.equal(saved.approvedBy,null)
},{timeout:120000})

test('CR5 live calendar priority keeps official holiday above schedule, branch and company rules',async t=>{
...
test('CR5 category rollback and concurrent changes keep every follower on the final mapping',async t=>{
 const chains=await f.repo('ApprovalChain').save([1,2,3].map(n=>({code:'R5ATOMIC'+n,nameAr:'Review atomic '+n,isActive:true})))
 await f.repo('ApprovalStep').save(chains.map(ch=>({chainId:ch.id,stepOrder:1,approverRole:'hr'})))
 const types=await f.repo('RequestType').save([1,2,3].map(n=>({code:'R5ATOMIC_TYPE'+n,nameAr:'Review atomic type '+n,category:'training',destinationHandler:'none',approvalChainId:chains[0].id,isActive:true})))
 await f.ok('PUT','/settings/request-categories/training/chain',{chainId:chains[0].id})
 await f.ds.query(`ALTER TABLE requests_config ADD CONSTRAINT CK_R5_CONFIG_FAILURE CHECK ([key] <> 'requests.category_chain.training' OR [value] <> '${chains[1].id}')`)
 let failure
 ...
 }finally{await f.ds.query('ALTER TABLE requests_config DROP CONSTRAINT CK_R5_CONFIG_FAILURE')}
