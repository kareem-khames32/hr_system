...
test('RC-UI-11: الخادم — مسارات بالصلاحيتين، ربط الفئة لكل الشركة، ومفتاح الربط مقفول في الإعدادات العامة', () => {
  ...
  const body = settings.slice(settings.indexOf('async upsertConfig('), settings.indexOf('// =====', settings.indexOf('async upsertConfig(')))
  const checks = ['assertCompanyWideWrite(user)', 'where: { key: dto.key }', 'dto.key = row.key', 'if (isCategoryChainKey(dto.key))', 'const licenceIssue = payrollSelfApprovalLicenceIssue']
  let previous = -1
  for (const check of checks) { const at = body.indexOf(check); assert.ok(at > previous, check); previous = at }
