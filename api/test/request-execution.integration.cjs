...
test('custody transfer enforces the officer branch and request owner before any write', async () => {
  ...
  assert.equal((await request(officer, 'POST', `/requests/custody/${assignment.id}/transfer`, { toEmployeeId: target.employee.id })).status, 403)
...
test('REQ-23 an out-of-branch structural manager sees no custody and cannot confirm it by ID', async () => {
  ...
  assert.equal((await request(leader.user, 'POST', `/custody/${assignment.id}/manager-confirm`)).status, 403)
