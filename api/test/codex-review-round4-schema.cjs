...
try{
  ...
  const indexes=(await pool.request().query(`SELECT t.name tableName,i.name indexName,i.type_desc,i.is_unique,i.has_filter,
    ...
    ('attendance_days','attendance_punches','attendance_exemptions','attendance_rule_versions','offboarding_cases','employee_salary_history_versions',
  ...
  const schema=(await pool.request().query(`SELECT CAST(SERVERPROPERTY('ProductVersion') AS varchar(30)) version,
    ...
    COL_LENGTH('dbo.payroll_items','paidPayMethod') paidPayMethodBytes, COL_LENGTH('dbo.users','scopeBranchIds') scopeBranchIdsBytes, COL_LENGTH('dbo.work_schedules','weekendExceptions') weekendExceptionsBytes, DATABASEPROPERTYEX(DB_NAME(),'Collation') collation, (SELECT COUNT(*) FROM dbo.app_schema_migrations) ledgerCount;
  ...
  fs.writeFileSync(path.join(__dirname,'codex-review-round4-schema-results.cjs'),'module.exports = '+JSON.stringify(result,null,2)+'\n')
