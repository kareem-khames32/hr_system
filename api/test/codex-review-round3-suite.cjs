...
stream.on('test:stdout', d => { const s=redact(d.message); stdout.push(s); if (/REVIEW_MEASURE|REVIEW_PROGRESS|cleanupVerified|BATCH_SHADOW|CR3_/.test(s)) console.log(s.trim()) })
...
stream.on('test:summary', d => {
  if (!d.file) {
    ...
    fs.writeFileSync(path.join(__dirname, `codex-review-round3-results-${suffix}.cjs`), 'module.exports = ' + redact(JSON.stringify(final, null, 2)) + '\n')
