...
function fixture({ periods = [period()], currentValue = { ...monthly, currency: 'EGP' }, revision = 1, noHistory = false, error = null, cycleStartDay = 1, legacy = false } = {}) {
  ...
  // عقد V2 يقرأ توقيت الإثبات من نص UTC صريح (createdAtUtc) كما يعيده SQL؛ الأداة كانت تنقصه فتُرفض المراجعة كتالفة.
  const header = { id: 7, employeeId: 3, revision, reason: 'إثبات شهر الراتب', evidenceReference: 'قرار اختبار', createdAt: new Date('2026-09-14T00:00:00Z'), createdAtUtc: '2026-09-14T00:00:00.000', createdBy: 4, currentSourceHash: salaryCurrentSourceHash(currentValue), contractVersion: legacy ? null : PAYROLL_MONTHLY_SALARY_HISTORY_VERSION, cycleStartDay: legacy ? null : cycleStartDay }
