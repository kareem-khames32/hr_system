...
const scope=require('./codex-review-round6-scope-evidence.cjs')
...
for(const file of fs.readdirSync(__dirname).filter(f=>/^codex-review-round6-results-.*\.cjs$/.test(f))){
...
async function main(){
  ...
  const originalRound5Changed=scope.originalEvidence.filter(x=>hash(fs.readFileSync(path.join(root,x.file)))!==x.sha256).map(x=>x.file)
  const diff=execFileSync('git',['diff','--name-only','508480b','--','api/src','src','docs/migrations'],{cwd:root,encoding:'utf8'}).trim()
  const scanFiles=fs.readdirSync(__dirname).filter(f=>f.startsWith('codex-review-round6-')).map(f=>path.join(__dirname,f))
  if(fs.existsSync(path.join(root,'CODEX_REVIEW_REPORT_6.md')))scanFiles.push(path.join(root,'CODEX_REVIEW_REPORT_6.md'))
  const findings=[]
  ...
  const output={checkedAt:new Date().toISOString(),head:execFileSync('git',['rev-parse','HEAD'],{cwd:root,encoding:'utf8'}).trim(),databaseCount:databases.length,databases,allDropped:databases.every(x=>x.dropRecorded&&x.absent),product,productDiffFromTarget:diff,originalRound5Changed,secretScan:{files:scanFiles.length,filesWithMatches:findings},staticChecks:{api:'tsc --noEmit --incremental false: exit 0',frontend:'Not rerun: no frontend product change'},performanceMeasured:false}
  fs.writeFileSync(path.join(__dirname,'codex-review-round6-final-audit-results.cjs'),'module.exports = '+JSON.stringify(output,null,2)+'\n')
  console.log(JSON.stringify({databaseCount:output.databaseCount,allDropped:output.allDropped,productUnchanged:product.every(x=>x.unchanged)&&!diff,originalRound5Changed,secretScan:output.secretScan}))
  assert.ok(output.allDropped);assert.ok(product.every(x=>x.unchanged));assert.equal(diff,'');assert.deepEqual(originalRound5Changed,[]);assert.deepEqual(findings,[])
}
