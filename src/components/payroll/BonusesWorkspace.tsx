...
export function BonusesWorkspace({ currency, mode, focusRequestId = null, initialTab, initialEmployeeId = null, initialPeriod = null }: {
  ...
  const loadRows = () => {
    ...
    fetchBonuses({ ...base, from: listRange.from, to: listRange.to })
      .then(async list => {
        // «بانتظار اعتمادي» أو فترة فاضية: عدد الطلبات برا الفترة يبان بدل ما تستخبى بصمت
        const everything = filters.view === 'pending_me' || list.length === 0 ? await fetchBonuses(base).catch(() => null) : null
  ...
  // موظف ليس مديرًا ولا معتمدًا: لا مساحة إدارية تُعرض له (ولا طلب ليه حتى برا الفترة)
...
function BonusList({ rows, loading, filters, setFilters, reload, currency, reasonMinLength, focusRequestId, range, setRange, context, outsideRange }: {
  ...
  const exportCsv = () => downloadCsv(`bonuses-${range ? `${dayRangeKey(range)}-` : ''}${csvDateStamp()}.csv`,
