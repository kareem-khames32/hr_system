...
export function PayrollAttendanceBreakdown({ item, currency }: { item: ApiPayrollItem; currency: string }) {
  let detail: { attendanceDeductions?: { policy?: { overlapPolicy: string }; days?: DayTrace[] };
    ...
    {relevant.length === 0 ? <p className="text-sm text-gray-500">لا توجد خصومات تأخير أو نقص ساعات أو أذونات مدفوعة في هذه النسخة.</p> :
      <div className="overflow-x-auto"><table className="w-full text-sm whitespace-nowrap">
        <thead><tr className="table-header">
          {['اليوم', 'التأخير قبل السماح (د)', 'النقص المرصود (د)', 'مغطى بإذن مدفوع (د)', 'تداخل مستبعد (د)', 'النقص المحاسب (د)', 'خصم التأخير', 'خصم النقص', 'خصم الإذن', 'تخفيض السقف', 'الإجمالي'].map(label => <th key={label} className="p-2 text-center">{label}</th>)}
        </tr></thead>
        <tbody>{relevant.map(day => <tr key={day.date} className="border-t border-gray-100">
          <td className="p-2" dir="ltr">{day.date}</td>
          {[day.unexcusedLateMinutes, day.rawShortfallMinutes, day.paidPermissionCoveredMinutes, day.overlapMinutes, day.chargeableShortfallMinutes].map((value, index) => <td key={index} className="p-2 text-center">{value}</td>)}
          {[day.latenessAmount, day.shortfallAmount, day.permissionAmount, day.cappedAmount, day.totalAmount].map((value, index) => <td key={index} className="p-2 text-center font-mono">{money(value)}</td>)}
        </tr>)}</tbody>
      </table></div>}
    <p className="text-xs text-gray-500">النقص المرصود معلومة حضور. الخصم يُحسب بعد استبعاد التداخل وتطبيق السماح والسقف اليومي. التقريب النهائي على إجمالي البند قد يُظهر فرق قرش عن جمع مبالغ الأيام المعروضة.</p>
  </section>
}
