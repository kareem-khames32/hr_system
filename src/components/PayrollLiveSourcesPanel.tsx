...
// المكونات السبعة للعرض؛ «ضغط العمل» بيتصرف من غير مؤثرات ومحرك السياسة وأسسه على الست بس
const salaryLabels: Record<string, string> = { basicSalary: 'الأساسي', housingAllowance: 'السكن', transportAllowance: 'الانتقال', phoneAllowance: 'الهاتف', workNatureAllowance: 'طبيعة العمل', otherAllowance: 'أخرى', workPressureAllowance: 'ضغط العمل' }
// راتب شهر المسير المختار: الست في salary (مدخل المحرك بنفس شكله) وبدل ضغط العمل في selectedSalary لوحده؛ التوثيق يكفيه الست
const effectSalaryKeys = Object.keys(salaryLabels).filter(key => key !== 'workPressureAllowance')
const segmentAmount = (row: Record<string, unknown>, key: string, workPressure: unknown) => key === 'workPressureAllowance' ? workPressure : liveSourceRecord(row.salary)?.[key]
