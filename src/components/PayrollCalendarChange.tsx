...
import { describeHolidayAudience, isHolidayAudience } from '@/lib/holiday-audience'
...
export function CalendarContextSummary({ context, loading = false, error = '' }: { context: PayrollCalendarContext | null; loading?: boolean; error?: string }) {
  ...
  return <div className="space-y-1 text-sm">
    ...
    {context.scope !== 'EMPLOYEE' && <details className="pt-2"><summary className="cursor-pointer font-medium text-blue-800">مراجعة جميع قيم التقويم التي سيشملها التأكيد</summary><div className="space-y-3 mt-3">
      {context.scope === 'GLOBAL' && <div><h4 className="font-medium mb-2">العطلات الحالية</h4>{holidayRows.length === 0 ? <p className="text-xs text-gray-500">لا توجد عطلات مسجلة.</p> : <ul className="space-y-2">{holidayRows.map((row, index) => <li key={index} className="rounded-lg bg-white/70 border border-gray-100 p-3"><p className="font-medium break-words">{text(row?.name)}</p><p className="text-xs text-gray-600 mt-1"><bdi dir="ltr">{text(row?.date)}</bdi> إلى <bdi dir="ltr">{row?.endDate === null ? text(row?.date) : text(row?.endDate)}</bdi> · {row?.country === null ? 'كل الدول' : text(row?.country)} · تسري على: {row?.audience === undefined ? 'للكل' : isHolidayAudience(row.audience) ? describeHolidayAudience(row.audience) : 'غير محدد'}</p></li>)}</ul>}</div>}
